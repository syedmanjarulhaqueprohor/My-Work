...
public class ScoreCardController {
    ...
    public void setScoreCardData(List<Player> innings1Batsman, List<Bowler> innings1Bowler,List<Player> innings2Batsman,List<Bowler> innings2Bowler) {
        ...
        batsmanTable.setItems(FXCollections.observableArrayList(innings1Batsman));
        bowlerTable.setItems(FXCollections.observableArrayList(innings1Bowler));
    }
    @FXML
    public void handleNextButton() {
        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("ScoreCard2.fxml"));
            Parent root = loader.load();
            ScoreCard2Controller controller2 = loader.getController();
            controller2.setMatchId(this.matchId);
            controller2.setTeamNames(this.teamAName, this.teamBName);
            controller2.setScoreCardData(i1Batsman, i1Bowler, i2Batsman, i2Bowler);
            Stage stage = (Stage) batsmanTable.getScene().getWindow();
            stage.setScene(new Scene(root));
            stage.setTitle("Innings 2 - Team B Scorecard");
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
